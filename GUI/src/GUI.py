...
def RunSingleIteration(i=1, NumProcessor=1):
    #PrintGreenString("sleep " + str(i % NumProcessor) +
    #                 " seconds " + str(i) + ", " + str(NumProcessor))
    print(type(NumProcessor))
    print(type(i))
    #time.sleep(i % NumProcessor)

    currentdir = os.getcwd()
    newFile = currentdir + "/MC_" + str(i).zfill(6)

    ggh = Path(newFile)
    ggh.mkdir(parents=True, exist_ok=True)
    os.chdir(newFile)

    DFNGEN_IF = True
    generate_or_empty_file("./log.txt")


    try:
        cpp_executable = cuDFNsys_GUI_path + "/DFN_Gen ../StochasticDFN"
        process = subprocess.Popen(cpp_executable, stdout=subprocess.PIPE,
                                   stderr=subprocess.STDOUT, universal_newlines=True, shell=True)

        # Iterate over the output of the process line by line
        for line in process.stdout:
            print(line, end="")
            # Check if the line contains the word "Warning"
            add_line_to_file("./log.txt", line)
            if "Warning" in line:
                PrintRedString("Warning found: " + line + ", MC_" + str(i))
                raise subprocess.CalledProcessError(
                    return_code, cpp_executable, "Error")
        # Wait for the process to finish
        return_code = process.wait()
    ...
    if DFNGEN_IF:
        try:
            cpp_executable = cuDFNsys_GUI_path + "/DFN_Mesh ../MeshPara"
            process = subprocess.Popen(cpp_executable, stdout=subprocess.PIPE,
                                       stderr=subprocess.STDOUT, universal_newlines=True, shell=True)
            ...
            print("Error running DFN_Mesh:", e)
            # Handle the error appropriately
            DFNMesh_IF = False

    if DFNMesh_IF:
        try:
            cpp_executable = cuDFNsys_GUI_path + "/DFN_Flow ../FlowPara"
            process = subprocess.Popen(cpp_executable, stdout=subprocess.PIPE,
                                       stderr=subprocess.STDOUT, universal_newlines=True, shell=True)
...
def RunMCIterations():
    ...
    def StartRunMC():
        ...
        if (NumProcessor == 1):
            label4 = Label(root, text="Serial MC iteration ...", fg="green")
            label4.pack()
            for i in range(1, NUMMC + 1, 1):
                RunSingleIteration(i, NumProcessor)

        if (NumProcessor > 1):
            label4 = Label(root, text=(
                "NumProcessor = " + str(NumProcessor) + ", Parallel MC iteration ... "), fg="green")
            label4.pack()

            with multiprocessing.Pool(processes=NumProcessor) as pool:
                # Map the function to the pool for each iteration
                pool.map(RunSingleIteration, [(i, NumProcessor) for i in range(1, NUMMC + 1)])
