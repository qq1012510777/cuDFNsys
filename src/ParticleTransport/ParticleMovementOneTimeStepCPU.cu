...
#include "ParticleTransport/ParticleMovementOneTimeStepCPU.cuh"

// ====================================================
// NAME:        ParticleMovementOneTimeStepCPU
// DESCRIPTION: ParticleMovementOneTimeStepCPU
// AUTHOR:      Tingchang YIN
// DATE:        15/11/2022
// ====================================================
template <typename T>
void cuDFNsys::ParticleMovementOneTimeStepCPU(unsigned long seed,
                                              T delta_T_,
                                              T Dispersion_local,
                                              ...
                                              int Nproc)
{
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        // ------------------move the particle
        // ------------------move the particle
        srand((unsigned long)(seed * (i + 1)));

        T z1 = cuDFNsys::RandomStandardNormal((T)rand() / (T)RAND_MAX, (T)rand() / (T)RAND_MAX),
          z2 = cuDFNsys::RandomStandardNormal((T)rand() / (T)RAND_MAX, (T)rand() / (T)RAND_MAX);
        ...
        for (uint Loop_time = 1; existLoop != true; Loop_time++)
        {
...
}; // ParticleMovementOneTimeStepCPU
template void cuDFNsys::ParticleMovementOneTimeStepCPU<double>(unsigned long seed,
                                                               double delta_T_,
                                                               ...
                                                               int Nproc);
template void cuDFNsys::ParticleMovementOneTimeStepCPU<float>(unsigned long seed,
                                                              float delta_T_,
