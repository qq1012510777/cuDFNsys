...
// ====================================================
template <typename T>
__global__ void cuDFNsys::ParticleMovementOneTimeStepGPUKernel(
    ...
    int2 *CorrespondingEleLocalEdge_device_ptr, size_t IfPureDiffusion, size_t IfReflectionAtInlet_)
{
    ...
    T z1 = curand_normal(&state), z2 = curand_normal(&state);

    // if (i != 198330 - 1)
    //     return;
    // // z1 = -3.7111978819675952578904798428993672132492;
    // // z2 = 3.4964017884433467031612963182851672172546;
    // InitPos.x = 17.2490805088697563007826829561963677406311;
    // InitPos.y = 12.1017185131185645019513685838319361209869;
    // EleID = 612;
    ...
    T ratio_diffusion = 1;
    T diffusion_term_x =
          (T)1.0f * z1 * sqrt((T)2.0f * Dispersion_local * delta_T_),
      diffusion_term_y =
          (T)1.0f * z2 * sqrt((T)2.0f * Dispersion_local * delta_T_);
    ...
    for (uint Loop_time = 1;; Loop_time++)
    {
        ...
        {
            Veloc_p = cuDFNsys::ReconstructVelocityGrid<T>(
                InitPos, Vertex_Triangle_ForVelocity, Veloc_triangle);
            ...
            normLastTrajectory =
                pow((TargPos.x - InitPos.x) * (TargPos.x - InitPos.x) +
                        (TargPos.y - InitPos.y) * (TargPos.y - InitPos.y),
                    0.5);
        }
        else
        {
        ...
        bool IfTargPosInGrid = cuDFNsys::IfPntInside2DConvexPoly<T>(
            TargPos, Vertex_Triangle_ForVelocity, 3);
        ...
        uint2 IfOnBorder = cuDFNsys::IfRandomWalkLieOnBorderTriangle<T>(
            TargPos, Vertex_Triangle_ForVelocity, (T)0.0);
        ...
        if (IfOnBorder.x == 1)
        {
            ...
            cuDFNsys::Vector3<T> Pos3D_ = cuDFNsys::Roate2DPositionTo3D<T>(
                TargPos, Frac_DEV[EleToFracID_ptr[EleID - 1]]);
            T *tmp_pnt = &(Pos3D_.x);
            if (abs(tmp_pnt[Dir_flow] - outletcoordinate) < 1e-4)
            {
                if (!If_periodic)
                    P_DEV[i].ParticleID =
                        -P_DEV[i].ParticleID; // reaches outlet plane
                // P_DEV[i].AccumDisplacement += pow((TargPos.x - InitPos.x) * (TargPos.x - InitPos.x) + (TargPos.y - InitPos.y) * (TargPos.y - InitPos.y), 0.5);
        ...
        uint result4[4] = {0};
        IFintersectBorder = cuDFNsys::WhichEdgeDoesTrajectoryIntersect<T>(
            Trajectory, Vertex_Triangle_ForVelocity, result4);
        ...
        if (!IFintersectBorder)
        {
            ...
            {
                T U_ratio =
                    (yy == MaxLoopInterect ? 0 : 0.001 / (pow(10.0, (T)yy)));
                ...
                IFintersectBorder =
                    cuDFNsys::WhichEdgeDoesTrajectoryIntersect<T>(
                        Trajectory, Vertex_Triangle_ForVelocity, result4);
            ...
            if (!IFintersectBorder)
            {
                ...
                Trajectory[0].x =
                    InitPos.x - (TargPos.x - InitPos.x) * 0.0000001;
                Trajectory[0].y =
                    InitPos.y - (TargPos.y - InitPos.y) * 0.0000001;
                IFintersectBorder =
                    cuDFNsys::WhichEdgeDoesTrajectoryIntersect<T>(
                        Trajectory, Vertex_Triangle_ForVelocity, result4);
                if (!IFintersectBorder)
                {
                    printf(
                        "Error: did not find which edge of the triangle that "
                        "intersects with the trajectory (particle ID: %d)!\n",
                        i + 1);
                    printf("Trajectory of particle ID %d:\n\t[%.40f, "
                           "%.40f]\n\t[%.40f, %.40f]\n",
                    ...
                    printf("Element ID %d of particle ID %d:\n\t[%.40f, "
                           "%.40f]\n\t[%.40f, %.40f]\n\t[%.40f, %.40f]\n",
                           EleID, i + 1, Vertex_Triangle_ForVelocity[0].x,
                           Vertex_Triangle_ForVelocity[0].y,
                           Vertex_Triangle_ForVelocity[1].x,
                           Vertex_Triangle_ForVelocity[1].y,
                           Vertex_Triangle_ForVelocity[2].x,
                           Vertex_Triangle_ForVelocity[2].y);
                    goto Debug100;
                }
            }
        }
        ...
        if (result4[0] > 1)
        {
            if (IFintersectBorder && result4[0] == 2)
            {
                ...
                // continue;
                printf("Error: the trajectory (particle ID: %d) may accross a "
                       "vertex of element %d? Impossible!\n",
                       i + 1, EleID);
                printf("Trajectory of particle ID %d:\n\t[%.40f, "
                       "%.40f]\n\t[%.40f, %.40f]\n",
                       i + 1, InitPos.x, InitPos.y, TargPos.x, TargPos.y);
            ...
            {
                printf("Error: the trajectory (particle ID: %d) intersects to "
                       "three edges of a triangle? Impossible!\n",
                       i + 1);
                printf("Trajectory of particle ID %d:\n\t[%.40f, "
                       "%.40f]\n\t[%.40f, %.40f]\n",
                       i + 1, InitPos.x, InitPos.y, TargPos.x, TargPos.y);
        ...
        cuDFNsys::Vector2<T> TheEdge[2] = {
            Vertex_Triangle_ForVelocity[result4[1]],
            Vertex_Triangle_ForVelocity[(result4[1] + 1) % 3]};

        cuDFNsys::Vector3<T> Inj =
            cuDFNsys::IntersectionLineLine2D<T>(TheEdge, Trajectory);

        if (Inj.x == 0)
        {
            printf("Error: the trajectory (particle ID: %d) is not "
            ...
            printf("The edge of particle ID %d intersected:\n\t[%.40f, "
                   "%.40f]\n\t[%.40f, %.40f]\n",
                   i + 1, TheEdge[0].x, TheEdge[0].y, TheEdge[1].x,
                   TheEdge[1].y);
            printf("Element ID %d of particle ID %d:\n\t[%.40f, "
                   "%.40f]\n\t[%.40f, %.40f]\n\t[%.40f, %.40f]\n",
                   EleID, i + 1, Vertex_Triangle_ForVelocity[0].x,
        ...
        cuDFNsys::Vector2<T> IntersectionOnEdge =
            cuDFNsys::MakeVector2(Inj.y, Inj.z);

        uint *tyu = &(EdgeNO.x);
        uint GlobalEdgeNO = tyu[result4[1]];

        uint NumOfElesSharedEdge =
            EdgesSharedEle_DEV[GlobalEdgeNO].NumSharedEle;

        if (NumOfElesSharedEdge == 1)
        {
            cuDFNsys::Vector3<T> Pos3D_ = cuDFNsys::Roate2DPositionTo3D<T>(
                IntersectionOnEdge, Frac_DEV[EleToFracID_ptr[EleID - 1]]);
            T *tmp_pnt = &(Pos3D_.x);
            ...
            if (abs(tmp_pnt[Dir_flow] - outletcoordinate) < 1e-4)
            {
                ...
                // if periodic at outlet
                // if periodic at outlet
                // if periodic at outlet
                if (CorrespondingEleLocalEdge_device_ptr[EleID - 1].x == -1 ||
                    CorrespondingEleLocalEdge_device_ptr[EleID - 1].y == -1)
                {

                    cuDFNsys::Vector3<T>
                        tmp1_o = cuDFNsys::Roate2DPositionTo3D<T>(
                            TheEdge[0], Frac_DEV[EleToFracID_ptr[EleID - 1]]),
                        tmp2_o = cuDFNsys::Roate2DPositionTo3D<T>(
                            TheEdge[1], Frac_DEV[EleToFracID_ptr[EleID - 1]]);

                    T *Utmp_ptr_t1 = &(tmp1_o.x), *Utmp_ptr_t2 = &(tmp2_o.x);
                    // printf("%.30f, %.30f\n", Utmp_ptr_t1[Dir_flow],
                    //        outletcoordinate);
                    // printf("%.30f, %.30f\n", Utmp_ptr_t2[Dir_flow],
                    //        outletcoordinate);

                    if (abs(Utmp_ptr_t1[Dir_flow] - outletcoordinate) < 1e-4 &&
                        abs(Utmp_ptr_t2[Dir_flow] - outletcoordinate) < 1e-4)
                    {
                        printf(
                            "At outlet, no periodic element for this element "
                            "(ID: %d)\n",
                            EleID);
                        goto Debug100;
                    }
                ...
                int corresponding_elementID =
                    CorrespondingEleLocalEdge_device_ptr[EleID - 1].x + 1;
                int LocalEdgeOnCEle =
                    CorrespondingEleLocalEdge_device_ptr[EleID - 1].y;

                cuDFNsys::Vector3<T> Triangle_inlet_coordinates[3];
                for (int j = 0; j < 3; ++j)
                    Triangle_inlet_coordinates[j] =
                        cuDFNsys::Roate2DPositionTo3D<T>(
                            cuDFNsys::MakeVector2(
                                Coordinate2D_Vec_dev_ptr
                                    [corresponding_elementID - 1]
                                        .x[j],
                                Coordinate2D_Vec_dev_ptr
                                    [corresponding_elementID - 1]
                                        .y[j]),
                            Frac_DEV[EleToFracID_ptr[corresponding_elementID -
                                                     1]]);

                // the particle meets the outlet, for periodic BC, the particle should be back to the inlet with the corresponding `the same` location
                //
                T norm_FinishedTrajec =
                    pow((IntersectionOnEdge.x - InitPos.x) *
                                (IntersectionOnEdge.x - InitPos.x) +
                            (IntersectionOnEdge.y - InitPos.y) *
                                (IntersectionOnEdge.y - InitPos.y),
                        0.5);
                T ratio_Trajectory = norm_FinishedTrajec / normLastTrajectory;
                delta_T_consume +=
                    (delta_T_ - delta_T_consume) * ratio_Trajectory;
                ratio_diffusion = (1.0 - delta_T_consume / delta_T_);

                // update the `InitPos`, this should be at the inlet
                // the 3D intersection point is `Pos3D_`
                // so the corresponding point is
                cuDFNsys::Vector3<T> Corre_Pos3D_init = Pos3D_;
                cuDFNsys::Vector3<T> Corre_Pos3D_Tar =
                    cuDFNsys::Roate2DPositionTo3D<T>(
                        TargPos, Frac_DEV[EleToFracID_ptr[EleID - 1]]);

                T *tmp_ont_corre_init = &(Corre_Pos3D_init.x);
                tmp_ont_corre_init[Dir_flow] += (-outletcoordinate * 2.0);

                tmp_ont_corre_init = &(Corre_Pos3D_Tar.x);
                tmp_ont_corre_init[Dir_flow] += (-outletcoordinate * 2.0);

                // now let's rotate the remainning trajectory
                // input: coordinates of a triangle: Triangle_inlet_coordinates
                //        local edge of rotation axis: LocalEdgeOnCEle (from 0)
                //   init and targ: Corre_Pos3D_init, Corre_Pos3D_Tar

                //cuDFNsys::Vector3<T> tmpydsf = Corre_Pos3D_Tar;

                bool DS = cuDFNsys::RotateLineSegToPlane<T>(
                    Corre_Pos3D_init, Corre_Pos3D_Tar,
                    Triangle_inlet_coordinates, LocalEdgeOnCEle,
                    outletcoordinate, Dir_flow);

                if (Corre_Pos3D_Tar.z < 5)
                {
                    printf("Point %d, appears in center, end: %.30f, %.30f, "
                           "%.30f\nFoot: %.30f, %.30f, %.30f\n",
                           P_DEV[i].ParticleID, Corre_Pos3D_Tar.x,
                           Corre_Pos3D_Tar.y, Corre_Pos3D_Tar.z,
                           Corre_Pos3D_init.x, Corre_Pos3D_init.y,
                           Corre_Pos3D_init.z);
                    goto Debug100;
                }
                ...
                if (!DS)
                {
                    printf("At OUTLET, cannot find perodic trajectory\n");
                    goto Debug100;
                }
                // rotate them to 2D

                InitPos = cuDFNsys::Roate3DPositionTo2D<T>(
                    Corre_Pos3D_init,
                    Frac_DEV[EleToFracID_ptr[corresponding_elementID - 1]]);
                TargPos = cuDFNsys::Roate3DPositionTo2D<T>(
                    Corre_Pos3D_Tar,
                    Frac_DEV[EleToFracID_ptr[corresponding_elementID - 1]]);

                FracID = (uint)EleToFracID_ptr[corresponding_elementID - 1];
                EleID = (uint)corresponding_elementID;
                EdgeNO =
                    make_uint3(EleID * 3 - 3, EleID * 3 - 2, EleID * 3 - 1);
                ...
                // printf("\nHaveRandomWalkerBeenThroughTrace: true\n");
                if (Dispersion_local != 0)
                    normLastTrajectory = pow(
                        (TargPos.x - InitPos.x) * (TargPos.x - InitPos.x) +
                            (TargPos.y - InitPos.y) * (TargPos.y - InitPos.y),
                        0.5);
                P_DEV[i].FactorPeriodic++;
                continue;
            }
            else if (tmp_pnt[Dir_flow] > ((T)-1.0 * outletcoordinate - 1e-4))
            {
                ...
                if (!If_periodic)
                    ...
                    else
                    {
                        ...
                        {
                            printf("Error: the trajectory (particle ID: %d) is "
                                   "acrossing the inlet, but the molecular "
                                   "diffusion is zero\n",
                                   i + 1);
                            printf("Trajectory of particle ID %d:\n\t[%.40f, "
                                   "%.40f]\n\t[%.40f, %.40f]\n",
                                   i + 1, InitPos.x, InitPos.y, TargPos.x,
                                   TargPos.y);
                            printf("The edge of particle ID %d "
                                   "intersected:\n\t[%.40f, %.40f]\n\t[%.40f, "
                                   "%.40f]\n",
                                   i + 1, TheEdge[0].x, TheEdge[0].y,
                                   TheEdge[1].x, TheEdge[1].y);
                            printf("Intersection of particle ID %d "
                                   "intersected:\n\t[%.40f, %.40f]\n",
                                   i + 1, IntersectionOnEdge.x,
                                   IntersectionOnEdge.y);
                            printf(
                                "Element ID %d of particle ID %d:\n\t[%.40f, "
                                "%.40f]\n\t[%.40f, %.40f]\n\t[%.40f, %.40f]\n",
                                EleID, i + 1, Vertex_Triangle_ForVelocity[0].x,
                                Vertex_Triangle_ForVelocity[0].y,
                                Vertex_Triangle_ForVelocity[1].x,
                        ...
                        else
                        {
                            T norm_FinishedTrajec =
                                pow((IntersectionOnEdge.x - InitPos.x) *
                                    ...
                                    0.5);
                            T ratio_Trajectory =
                                norm_FinishedTrajec / normLastTrajectory;
                            delta_T_consume +=
                                (delta_T_ - delta_T_consume) * ratio_Trajectory;
                            ratio_diffusion =
                                (1.0 - delta_T_consume / delta_T_);
                    ...
                    CorrespondingEleLocalEdge_device_ptr[EleID - 1].y == -1)
                {
                    ...
                    if (abs(Utmp_ptr_t1[Dir_flow] - (-1.) * outletcoordinate) <
                            1e-4 &&
                        abs(Utmp_ptr_t2[Dir_flow] - (-1.) * outletcoordinate) <
                            1e-4)
                    {
                        printf("At inlet, no periodic element for this element "
                               "(ID: %d)\n",
                               EleID);
                        goto Debug100;
                    }
                ...
                if (!DS)
                {
                    printf("At, INLET, cannot find perodic trajectory\n");
                    goto Debug100;
                }
                // rotate them to 2D
            ...
            NON_FLUX_EDGE:;
                ...
                //----------------
                TargPos = cuDFNsys::ParticleReflection<T>(
                    TargPos, Vertex_Triangle_ForVelocity[result4[1]],
                    Vertex_Triangle_ForVelocity[(result4[1] + 1) % 3]);
                InitPos = IntersectionOnEdge;
        ...
        else if (NumOfElesSharedEdge == 2)
        {
            ...
            InitPos = IntersectionOnEdge;
            EleID = (EdgesSharedEle_DEV[GlobalEdgeNO].EleID[0] == EleID
                         ? EdgesSharedEle_DEV[GlobalEdgeNO].EleID[1]
                         : EdgesSharedEle_DEV[GlobalEdgeNO].EleID[0]);

            // P_DEV[i].AccumDisplacement += norm_FinishedTrajec;

            // ttry to update when the random walker is purely advection-driven, but this is not good
            if ((HaveRandomWalkerBeenThroughTrace ||
                 HaveRandomWalkerBeenReflected) &&
                Dispersion_local == 0)
                IfUpdateTrajectoryLastStep = true; // so I change this to false

            // if the random walker has not been reflected or through traces, the trajectory will be updated next round
            // otherwise, just check which element does the target point lie in
        ...
        else
        {
            ...
            // determine where to go
            int IndexLocal =
                -1; // index of local shared edge NO the next element;
            ...
            cuDFNsys::WhichElementToGo<T>(
                EleID, NumOfElesSharedEdge, Dispersion_local,
            ...
            {
                printf(
                    "Warning: illeagal indice appear, because the "
                    "determination of which element to go is "
                    "failed!\nnewELEID_: %d\nnewFracID_: %d\nIndexLocal: %d\n",
                    newELEID_, newFracID_, IndexLocal);
                goto Debug100;
            }
            ...
            if (ifAllsharedEdgeVelocityPositive == true)
            {
                ...
                InitPos = IntersectionOnEdge;
                TargPos = cuDFNsys::ParticleReflection<T>(TargPos, TheEdge[0],
                                                          TheEdge[1]);
            ...
            EdgeNO = make_uint3(EleID * 3 - 3, EleID * 3 - 2,
                                EleID * 3 - 1); // from 0
            ...
            cuDFNsys::Vector3<T> IntersectionOnEdge3D = cuDFNsys::MakeVector3(
                IntersectionOnEdge.x, IntersectionOnEdge.y, (T)0);
            IntersectionOnEdge3D =
                cuDFNsys::ProductSquare3Vector3<T>(RK_1, IntersectionOnEdge3D);
            IntersectionOnEdge3D = cuDFNsys::MakeVector3(
                IntersectionOnEdge3D.x + Frac_DEV[PreviousFracID].Center.x,
                IntersectionOnEdge3D.y + Frac_DEV[PreviousFracID].Center.y,
                IntersectionOnEdge3D.z + Frac_DEV[PreviousFracID].Center.z);

            cuDFNsys::Vector3<T> IntersectionOnEdge3D_anotherFrac =
                cuDFNsys::MakeVector3(
                    IntersectionOnEdge3D.x - Frac_DEV[FracID].Center.x,
            ...
            IntersectionOnEdge3D_anotherFrac =
                cuDFNsys::ProductSquare3Vector3<T>(
                    RK_2, IntersectionOnEdge3D_anotherFrac);
            cuDFNsys::Vector2<T> IntersectionOnEdge2D_anotherFrac =
                cuDFNsys::MakeVector2(IntersectionOnEdge3D_anotherFrac.x,
                                      IntersectionOnEdge3D_anotherFrac.y);
            //---------------------------------------------------
            // TheEdge 3D
            cuDFNsys::Vector3<T> TheEdge_oldEle[2];
            TheEdge_oldEle[0] =
                cuDFNsys::MakeVector3(TheEdge[0].x, TheEdge[0].y, (T)0);
            TheEdge_oldEle[1] =
                cuDFNsys::MakeVector3(TheEdge[1].x, TheEdge[1].y, (T)0);
            TheEdge_oldEle[0] =
                cuDFNsys::ProductSquare3Vector3<T>(RK_1, TheEdge_oldEle[0]);
            TheEdge_oldEle[1] =
                cuDFNsys::ProductSquare3Vector3<T>(RK_1, TheEdge_oldEle[1]);
            TheEdge_oldEle[0] = cuDFNsys::MakeVector3(
                TheEdge_oldEle[0].x + Frac_DEV[PreviousFracID].Center.x,
                TheEdge_oldEle[0].y + Frac_DEV[PreviousFracID].Center.y,
                TheEdge_oldEle[0].z + Frac_DEV[PreviousFracID].Center.z);
            TheEdge_oldEle[1] = cuDFNsys::MakeVector3(
                TheEdge_oldEle[1].x + Frac_DEV[PreviousFracID].Center.x,
            ...
            TheEdge_newEle[0] =
                cuDFNsys::MakeVector3(TriangleNew_ele[IndexLocal].x,
                                      TriangleNew_ele[IndexLocal].y, (T)0);
            TheEdge_newEle[1] = cuDFNsys::MakeVector3(
                TriangleNew_ele[(IndexLocal + 1) % 3].x,
                TriangleNew_ele[(IndexLocal + 1) % 3].y, (T)0);

            T RK_3[3][3];
            Frac_DEV[FracID].RoationMatrix(RK_3, 23);
            TheEdge_newEle[0] =
                cuDFNsys::ProductSquare3Vector3<T>(RK_3, TheEdge_newEle[0]);
            TheEdge_newEle[1] =
                cuDFNsys::ProductSquare3Vector3<T>(RK_3, TheEdge_newEle[1]);
            TheEdge_newEle[0] = cuDFNsys::MakeVector3(
                TheEdge_newEle[0].x + Frac_DEV[FracID].Center.x,
                TheEdge_newEle[0].y + Frac_DEV[FracID].Center.y,
                TheEdge_newEle[0].z + Frac_DEV[FracID].Center.z);
            TheEdge_newEle[1] = cuDFNsys::MakeVector3(
                TheEdge_newEle[1].x + Frac_DEV[FracID].Center.x,
            ...
            if (abs(sqrt((TheEdge_oldEle[0].x - TheEdge_newEle[0].x) *
                             (TheEdge_oldEle[0].x - TheEdge_newEle[0].x) +
                         (TheEdge_oldEle[0].y - TheEdge_newEle[0].y) *
                             (TheEdge_oldEle[0].y - TheEdge_newEle[0].y))) <
                    1e-3 &&
                abs(sqrt((TheEdge_oldEle[1].x - TheEdge_newEle[1].x) *
                             (TheEdge_oldEle[1].x - TheEdge_newEle[1].x) +
                         (TheEdge_oldEle[1].y - TheEdge_newEle[1].y) *
                             (TheEdge_oldEle[1].y - TheEdge_newEle[1].y))) <
                    1e-3)
            {
                IfTwoEdgesOpposite = false;
            }
            else if (abs(sqrt((TheEdge_oldEle[0].x - TheEdge_newEle[1].x) *
                                  (TheEdge_oldEle[0].x - TheEdge_newEle[1].x) +
                              (TheEdge_oldEle[0].y - TheEdge_newEle[1].y) *
                                  (TheEdge_oldEle[0].y -
                                   TheEdge_newEle[1].y))) < 1e-3 &&
                     abs(sqrt((TheEdge_oldEle[1].x - TheEdge_newEle[0].x) *
                                  (TheEdge_oldEle[1].x - TheEdge_newEle[0].x) +
                              (TheEdge_oldEle[1].y - TheEdge_newEle[0].y) *
                                  (TheEdge_oldEle[1].y -
                                   TheEdge_newEle[0].y))) < 1e-3)
            {
            ...
            {
                printf(
                    "Error: particle ID: %d, when a random walker get through "
                    "a trace, the shared edge is not accurately identified\n",
                    i + 1);
                goto Debug100;
            }

            // the new edge in 2D
            cuDFNsys::Vector2<T> TheEdge_new[2] = {
                cuDFNsys::MakeVector2(TriangleNew_ele[IndexLocal].x,
                                      TriangleNew_ele[IndexLocal].y),
                cuDFNsys::MakeVector2(TriangleNew_ele[(IndexLocal + 1) % 3].x,
                                      TriangleNew_ele[(IndexLocal + 1) % 3].y)};
            ...
            if (!IfTwoEdgesOpposite)
            {
                TheEdge_new[1] =
                    cuDFNsys::MakeVector2(TriangleNew_ele[IndexLocal].x,
                                          TriangleNew_ele[IndexLocal].y);
                TheEdge_new[0] = cuDFNsys::MakeVector2(
                    TriangleNew_ele[(IndexLocal + 1) % 3].x,
                    TriangleNew_ele[(IndexLocal + 1) % 3].y);
            }

            // the angle between the old edge and the old remainning trajectory
            cuDFNsys::Vector2<T> vector_1_edge = cuDFNsys::MakeVector2(
                                     TheEdge[1].x - TheEdge[0].x,
                                     TheEdge[1].y - TheEdge[0].y),
                                 vector_1_trajectOld = cuDFNsys::MakeVector2(
                                     TargPos.x - IntersectionOnEdge.x,
                                     TargPos.y - IntersectionOnEdge.y);
            T norm_1_edge = sqrt(vector_1_edge.x * vector_1_edge.x +
                                 vector_1_edge.y * vector_1_edge.y);
            T norm_2_trjOld =
                sqrt(vector_1_trajectOld.x * vector_1_trajectOld.x +
                     vector_1_trajectOld.y * vector_1_trajectOld.y);
            vector_1_edge.x /= norm_1_edge, vector_1_edge.y /= norm_1_edge;
            vector_1_trajectOld.x /= norm_2_trjOld,
                vector_1_trajectOld.y /= norm_2_trjOld;
            // https://wumbo.net/formulas/angle-between-two-vectors-2d/
            // angle amount from vector 1 to vector 2
            T angle_s = atan2(vector_1_trajectOld.y * vector_1_edge.x -
                                  vector_1_trajectOld.x * vector_1_edge.y,
                              vector_1_trajectOld.x * vector_1_edge.x +
                                  vector_1_trajectOld.y * vector_1_edge.y);

            // map it to the new element
            cuDFNsys::Vector2<T> vector_2_edge =
                cuDFNsys::MakeVector2(TheEdge_new[1].x - TheEdge_new[0].x,
                                      TheEdge_new[1].y - TheEdge_new[0].y);
            T norma_2_edge = sqrt(vector_2_edge.x * vector_2_edge.x +
                                  vector_2_edge.y * vector_2_edge.y);
        ...
        reverseDir:;
            ...
            qua = qua.DescribeRotation(cuDFNsys::MakeVector3((T)0, (T)0, (T)1),
                                       angle_s);
            cuDFNsys::Vector3<T> temp_new_tra = qua.Rotate(
                cuDFNsys::MakeVector3(vector_2_edge.x, vector_2_edge.y, (T)0));

            cuDFNsys::Vector2<T> newTarget =
                cuDFNsys::MakeVector2(temp_new_tra.x, temp_new_tra.y);
            T norm_new_targ =
                sqrt(newTarget.x * newTarget.x + newTarget.y * newTarget.y);
            ...
            newTarget.x = IntersectionOnEdge2D_anotherFrac.x +
                          newTarget.x * norm_2_trjOld;
            newTarget.y = IntersectionOnEdge2D_anotherFrac.y +
                          newTarget.y * norm_2_trjOld;
            ...
            {
                // vector_2_edge
                cuDFNsys::Vector2<T> vector_2_ase = cuDFNsys::MakeVector2(
                    TargPos.x - InitPos.x, TargPos.y - InitPos.y);
                T norm_vector_2_ase = sqrt(vector_2_ase.x * vector_2_ase.x +
                                           vector_2_ase.y * vector_2_ase.y);
                vector_2_ase.x /= norm_vector_2_ase,
                    vector_2_ase.y /= norm_vector_2_ase;

                // one
                cuDFNsys::Vector2<T> vector_3_ase = cuDFNsys::MakeVector2(
                    TriangleNew_ele[(IndexLocal + 2) % 3].x - TheEdge_new[0].x,
                    TriangleNew_ele[(IndexLocal + 2) % 3].y - TheEdge_new[0].y);
                norm_vector_2_ase = sqrt(vector_3_ase.x * vector_3_ase.x +
                                         vector_3_ase.y * vector_3_ase.y);
                vector_3_ase.x /= norm_vector_2_ase,
                    vector_3_ase.y /= norm_vector_2_ase;

                // angle
                T angle_u1 = atan2(vector_2_ase.y * vector_2_edge.x -
                                       vector_2_ase.x * vector_2_edge.y,
                                   vector_2_ase.x * vector_2_edge.x +
                                       vector_2_ase.y * vector_2_edge.y);
                T angle_u2 = atan2(vector_3_ase.y * vector_2_edge.x -
                                       vector_3_ase.x * vector_2_edge.y,
                                   vector_3_ase.x * vector_2_edge.x +
                                       vector_3_ase.y * vector_2_edge.y);
                if (cuDFNsys::Sgn<T>(angle_u1) != cuDFNsys::Sgn<T>(angle_u2) &&
                    !IfHasBeenReverse)
                {
                    angle_s = -angle_s;
    ...
    TimeStepInfo:;
        ...
        if (P_DEV[i].ParticleID >= 0)
            TargPos3D = cuDFNsys::MakeVector3(P_DEV[i].Position2D.x,
                                              P_DEV[i].Position2D.y, (T)0.0);
        else
        ...
        // pre position
        cuDFNsys::Vector3<T> Init_pre_3D =
            cuDFNsys::MakeVector3(Pre_initPos.x, Pre_initPos.y, (T)0.0);
        ...
        P_DEV[i].AccumDisplacement +=
            sqrt((TargPos3D.x - Init_pre_3D.x) * (TargPos3D.x - Init_pre_3D.x) +
                 (TargPos3D.y - Init_pre_3D.y) * (TargPos3D.y - Init_pre_3D.y) +
                 (TargPos3D.z - Init_pre_3D.z) * (TargPos3D.z - Init_pre_3D.z));
        //printf("%.30f\n", P_DEV[i].AccumDisplacement);

        // if (stepNO > 100 && P_DEV[i].FactorPeriodic == 0)
        //     goto Debug100;
    ...
    Debug100:;
        Particle_runtime_error_dev_pnt[i] = 1;
        printf("Particle ID: %d, EleID (from 1): %d, P_DEV[i].Position2D: "
               "%.40f, %.40f,\nz1: %.40f\nz2: %.40f\n",
               P_DEV[i].ParticleID, P_DEV[i].ElementID, P_DEV[i].Position2D.x,
               P_DEV[i].Position2D.y, z1, z2);
        return;
    }
}; // ParticleMovementOneTimeStepGPUKernel
template __global__ void cuDFNsys::ParticleMovementOneTimeStepGPUKernel<double>(
    unsigned long seed, double delta_T_, double Dispersion_local,
    cuDFNsys::Particle<double> *P_DEV, cuDFNsys::Fracture<double> *Frac_DEV,
    cuDFNsys::EdgeToEle *EdgesSharedEle_DEV,
