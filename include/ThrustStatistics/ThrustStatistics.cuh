...
// Functor to calculate the mean of elements in a vector
template <typename T>
...
// Functor to calculate the squared difference from the mean
template <typename T>
struct SquaredDifference
{
    T mean;
    __host__ __device__ SquaredDifference(T _mean) : mean(_mean) {}
                                     ...
                                     summary_stats_data<T>>
{
    ...
    operator()(const summary_stats_data<T> &x,
               const summary_stats_data<T> &y) const
    {
        summary_stats_data<T> result;
        ...
        result.M4 = x.M4 + y.M4;
        result.M4 +=
            delta4 * x.n * y.n * (x.n * x.n - x.n * y.n + y.n * y.n) / n3;
        result.M4 +=
            (T)6.0 * delta2 * (x.n * x.n * y.M2 + y.n * y.n * x.M2) / n2;
...
int main(void)
{
    ...
    summary_stats_data<T> result = thrust::transform_reduce(
        d_x.begin(), d_x.end(), unary_op, init, binary_op);
