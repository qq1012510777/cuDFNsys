...
int main(int argc, char *argv[])
{
    try
    {
        ...
        if (Percolation_cluster.size() > 0)
        {
            ...
            cout << "size of host_vector of cuDFNsys::Fracture: " << Frac_verts_host.size() << endl;
            cuDFNsys::RemoveDeadEndFrac RDEF{Fracs_percol,
                                             IntersectionPair_percol,
                                             (size_t)perco_dir,
                                             Frac_verts_host,
                                             Intersection_map};
            cout << "after removeing dead fractures, size of host_vector of cuDFNsys::Fracture: " << Frac_verts_host.size() << endl;
            for (size_t i = 0;; ++i)
            {
