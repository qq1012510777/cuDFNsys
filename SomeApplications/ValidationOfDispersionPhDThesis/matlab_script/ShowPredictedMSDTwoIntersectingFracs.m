...
title_matrix = ["(a)", "(b)"; "(c) Pe = 300", "(d) Pe = 0.1"];
Filename_Ss = ["TwoIntersectingFrac", "TwoNonintersectingFrac"; ...
    "TwoIntersectingFracPe_0.1", "TwoNonintersectingFracPe_0.1";];
for i = 1:2
    Filepathss_tt = [Filepathss, '/', char(Filename_Ss(1, i))];
...
for i = 1:2
    for il = 1:2
        Filepathss_tt = [Filepathss, '/', char(Filename_Ss(i, il))]
        ...
        elseif (il == 2)
            scatter(A, B, 's', 'MarkerEdgeColor', 'b'); hold on
        end
    
        if (il == 1)
            plot(A, A .* 2 .* Dispersion_local .* (cos(Theta_s(i) / 180 * pi)) .^ 2 + B(1), 'r-'); hold on
        end
