...
__global__ void Transform2DTO3DKernel(
    ...
    _DataType_ *Position3D_dev_ptr, _DataType_ *temp2Dpos_dev_ptr,
    uint *ElementFracTag_cuda_devptr, uint *EleTag_device_ptr, uint count)
{
    ...
    cuDFNsys::Vector3<_DataType_> Pos = cuDFNsys::MakeVector3(
        temp2Dpos_dev_ptr[idx], temp2Dpos_dev_ptr[idx + numParticles],
        (_DataType_)0.0);
    //if (idx == 0)
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        Frac_verts_device_ptr =
            thrust::raw_pointer_cast(Frac_verts_device.data());
        ...
        thrust::device_vector<uint> ElementFracTag_cuda_dev =
            ElementFracTag_cuda;
        ...
        for (uint i = (If_FPT == false ? 0 : ExistingNumsteps);
             i <= ExistingNumsteps; ++i)
        {
            ...
            {
                filename = ParticlePosition + "LastStep.h5",
            ...
            try
            {
                temp2Dpos = h5g.ReadDataset<_DataType_>(
                    filename, "N",
                    "Step_" + cuDFNsys::ToStringWithWidth(i, 10));
            }
            catch (...)
            {
                continue;
            ...
            thrust::host_vector<_DataType_> temp2DposCUDA(
                data_s, data_s + temp2Dpos.size());
            thrust::device_vector<_DataType_> temp2DposCUDA_dev = temp2DposCUDA;
            _DataType_ *temp2DposCUDA_dev_ptr;
            temp2DposCUDA_dev_ptr =
                thrust::raw_pointer_cast(temp2DposCUDA_dev.data());
            uint numParticles = temp2Dpos.size() / 2;
            // cout << "numParticles: " << numParticles << endl;

            vector<uint> EleTag =
                h5g.ReadDataset<uint>(filename, "N",
                                      "ParticleIDAndElementTag_" +
                                          cuDFNsys::ToStringWithWidth(i, 10));
            uint *data_y = EleTag.data();
            thrust::host_vector<uint> EleTag_host(data_y + EleTag.size() / 2,
                                                  data_y + EleTag.size());
            ...
            uint *EleTag_device_ptr =
                thrust::raw_pointer_cast(EleTag_device.data());
            ...
            _DataType_ *Position3D_dev_ptr =
                thrust::raw_pointer_cast(Position3D_dev.data());

            Transform2DTO3DKernel<<<numParticles / 256 + 1, 256 /*  1, 2*/>>>(
                Frac_verts_device_ptr, Position3D_dev_ptr,
                temp2DposCUDA_dev_ptr, ElementFracTag_cuda_devptr,
                EleTag_device_ptr, numParticles);
            cudaDeviceSynchronize();
            ...
            h5g.AddDataset(outfilename, "N",
                           "Step_" + cuDFNsys::ToStringWithWidth(i, 10),
                           Position3D.data(), dim_pos);
