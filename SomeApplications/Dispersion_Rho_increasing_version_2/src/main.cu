...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i <= NumFracIncre; ++i)
    {
        ...
        system(("mkdir ./" + DFNFileName + " -p").c_str());
        ...
        if (!IfAFileExist("./" + DFNFileName + "/Class_DFN.h5"))
        {
            ...
            CreateOrEmptyFile(DFNFileName + "/" + DFN_Gen_csv_name + ".csv");

            AddLineToFile("./" + DFNFileName + "/" + DFN_Gen_csv_name + ".csv",
                          "IfStochastic,1,\n");
            AddLineToFile("./" + DFNFileName + "/" + DFN_Gen_csv_name + ".csv",
            ...
            AddLineToFile("./" + DFNFileName + "/" + DFN_Gen_csv_name + ".csv",
                          "Percolation_direction," + std::to_string(PercoDir) +
                              ",\n");
            AddLineToFile("./" + DFNFileName + "/" + DFN_Gen_csv_name + ".csv",
                          "NumFractureGroups,1,\n");
            AddLineToFile("./" + DFNFileName + "/" + DFN_Gen_csv_name + ".csv",
            ...
            AddLineToFile("./" + DFNFileName + "/" + DFN_Gen_csv_name + ".csv",
                          "Gamma," +
                              DoubleNumberToScientificNotationString(Gamma) +
                              ",\n");
            ...
            for (int j = 0; j < 10000; ++j)
            {
                ...
                if (!DFN_gen_success || PercolationClustersSize == 0)
                {
                    ...
                    system(
                        ("rm -rf ./" + DFNFileName + "/Class_DFN.h5").c_str());
                    continue;
                }
            ...
            !IfAFileExist("./" + DFNFileName + "/Class_MESH.h5"))
        {
            ...
            CreateOrEmptyFile(DFNFileName + "/" + DFN_Mesh_csv_name + ".csv");

            AddLineToFile("./" + DFNFileName + "/" + DFN_Mesh_csv_name + ".csv",
            ...
            !IfAFileExist("./" + DFNFileName + "/Class_FLOW.h5"))
        {
            ...
            CreateOrEmptyFile(DFNFileName + "/" + DFN_flow_csv_name + ".csv");

            AddLineToFile("./" + DFNFileName + "/" + DFN_flow_csv_name + ".csv",
                          "MuOverRhoG,1,\n");
            AddLineToFile("./" + DFNFileName + "/" + DFN_flow_csv_name + ".csv",
            ...
            if (!DFN_flow_success)
            {
                ...
                system(("rm -rf ./" + DFNFileName + "/Class_DFN.h5 ./" +
                        DFNFileName + "/Class_MESH.h5 ./" + DFNFileName +
                        "/Class_FLOW.h5")
                           .c_str());
                i--;
                continue;
            ...
            IfAFileExist("./" + DFNFileName + "/Class_FLOW.h5"))
        {
            ...
            //     write PT csv
            {
                ...
                tmp = h5g.ReadDataset<double>(
                    "./" + DFNFileName + "/Class_FLOW.h5", "N", "MeanVelocity");
                double meanV = tmp[0];

                tmp = h5g.ReadDataset<double>(
                    "./" + DFNFileName + "/Class_FLOW.h5", "N", "MaxVelocity");
                double maxV = tmp[0];

                tmp = h5g.ReadDataset<double>(
                    "./" + DFNFileName + "/Class_MESH.h5", "N", "MeanGridSize");
                    ...
                    MolecularDiffusionCoefficient == 0)
                {
                    ...
                    cout << DFNFileName
                         << ": advection only, DeltaT = " << AdvectionTimeScale
                         << " / " << FactorReduceCharacteristicTimescale
                         << " = " << DeltaT << endl;
                }
                else
                ...
                CreateOrEmptyFile(DFNFileName + "/" + DFN_PT_csv_name + ".csv");

                AddLineToFile(
                    "./" + DFNFileName + "/" + DFN_PT_csv_name + ".csv",
                    "NumParticles," + std::to_string(NumberParticles) + ",\n");
                AddLineToFile(
                    "./" + DFNFileName + "/" + DFN_PT_csv_name + ".csv",
                ...
                AddLineToFile(
                    ...
                    "If_OutputAllPTInformationOrFPTCurve," +
                        std::to_string(If_OutputAllPTInformationOrFPTCurve) +
                        ",\n");
            ...
            if (!DFN_PT_success)
            {
                ...
                {
                    tmp_count = h5g.ReadDataset<uint>("./" + DFNFileName +
                                                          "/PT_ErrorCount.h5",
                                                      "N", "ErrorCount");
                    tmp_count[0]++;
                    h5g.OverWrite("./" + DFNFileName + "/PT_ErrorCount.h5", "N",
                                  "ErrorCount", tmp_count.data(),
                                  make_uint2(1, 0));
                }
                cout << DFNFileName
