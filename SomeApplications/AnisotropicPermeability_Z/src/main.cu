...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i <= NumFracIncre; ++i)
    {
        ...
        string DFNFileName = "DFN_Z_" + cuDFNsys::ToStringWithWidth(i + 1, 3);

        string DFNFileName_X = "DFN_X_" + cuDFNsys::ToStringWithWidth(i + 1, 3);
        string DFNFileName_Y = "DFN_Y_" + cuDFNsys::ToStringWithWidth(i + 1, 3);
        ...
        try
        {
            if (IfAFileExist("./" + DFNFileName + "/Z_DarcyFlow_Finished"))
                continue;
        ...
        catch (cuDFNsys::ExceptionsIgnore e)
        {
            ...
            result_system = system(
                ("rm -rf ./" + DFNFileName + "/*.h5 " + DFNFileName +
                 "/ParticlePositionResult/* ./" + DFNFileName + "/Z_DarcyFlow_Finished")
                    .c_str());
            i--;
            continue;
        }
